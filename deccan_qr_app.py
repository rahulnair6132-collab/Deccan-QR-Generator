...
def sidebar_header():
    with st.sidebar:
        if os.path.exists(LOGO_PATH):
            ...
            st.markdown(f"<h2 style='color:{DECCAN_BLUE};margin:0'>DECCAN</h2>"
                        f"<div style='color:{DECCAN_GREY};font-size:12px'>SINCE 1966</div>",
...
def label_with_caption(code_img, caption, sku="", target_px=QR_TARGET_PX,
    ...
    canvas.paste(code_img, ((canvas_w - target_px) // 2, 0))
...
def build_outputs(jobcard, sku, mr_jobs, start=1):
    ...
    for mr, qty in mr_jobs:
        ...
        for _ in range(qty):
            ...
            payload = f"{jobcard}|{mr}|{serial}"
            img = label_with_caption(make_qr_image(payload), serial, sku=sku)
            images.append((mr, serial, img))
            rows.append({"InsulatorID": serial, "JobCard": jobcard, "MRNo": mr,
                         "SKU": sku, "QRPayload": payload,
    ...
    return rows, images, xlsx_bytes, csv_bytes, zip_buf.getvalue()
...
def prepare_data(df, do_shift_repair=True, do_md_fix=True):
    """Clean + type the raw export. Returns (df, notes list)."""
    notes = []
    df = df.copy()
...
def page_visualizer():
    ...
    df, notes = prepare_data(raw, do_shift_repair=do_shift, do_md_fix=do_md)
    for n in notes:
        st.info("🛠 " + n)
    ...
    m1.metric("Insulators in file", f"{df['Title'].nunique()}")
