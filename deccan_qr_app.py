...
def render_charts(stage_frames, reused_all):
    ...
    document.getElementById('btnPdf').onclick = async () => {{
      try {{
        const imgs = await renderImgs();
        // A4 portrait in points: 595 x 842, with margins
        ...
        pdf.text(TITLE, PW / 2, M + 10, {{align: 'center'}});
        pdf.setTextColor(107, 114, 128);
        pdf.setFont('helvetica', 'normal'); pdf.setFontSize(10);
        pdf.text(SUB, PW / 2, M + 28, {{align: 'center'}});
        let first = true;
        for (const im of imgs) {{
          if (!first) pdf.addPage();
          const topPad = first ? M + 50 : M;
          first = false;
          const availW = PW - M * 2, availH = PH - topPad - M;
