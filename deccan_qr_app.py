...
def build_stage_frames(df, win_start, win_end, machine_map=None):
    ...
    for s in STAGES:
        ...
        if det.empty:
            ...
            continue
        det = det.sort_values(["JobCard", "InsulatorID"]).reset_index(drop=True)
        ...
        summ["OK Qty"] = (summ["Total Produced Qty"] - summ["Rework Qty"]
                          - summ["Rejection Qty"])
        summ = summ[["Job Card", "Total Produced Qty", "OK Qty",
        ...
        if not det.empty:
            for jc, grp_df in det.groupby("JobCard", sort=True):
                ...
                for _, row in grp_df.iterrows():
                    ...
                    rr += 1
                rr += 1
        for w_, col in zip([22, 13, 15, 11, 11, 12, 11, 9, 13, 16, 15, 17, 34],
...
def render_charts(stage_frames, reused_all):
    ...
    c5, c6 = st.columns(2)
    with c5:
        g = fdf.groupby("Status").size().reset_index(name="Qty")
        donut = alt.Chart(g).mark_arc(innerRadius=70).encode(
            ...
            title=f"Quality Split — {len(fdf)} unit-stage records", height=320)
...
def page_visualizer():
    ...
    excl = st.multiselect(
        "Excluded insulators (auto-detected duplicates pre-selected — add or remove "
        "any ID; e.g. remove one if it is actually a real unit)",
        every_id, default=detected)
    if not dup_df.empty:
        with st.expander(f"Why these {len(dup_df)} were auto-detected as duplicates"):
            st.dataframe(dup_df[["Title", "DuplicateOf", "JobCard", "MRNo"]],
                         width="stretch", hide_index=True)
    full = pd.concat([df_all, dup_df.drop(columns=["DuplicateOf"], errors="ignore")],
    ...
    stage_frames, reused_win, reused_all, unattr = build_stage_frames(
        df, win_start, win_end, machine_map=mmap)
    ...
    total_units = sum(len(d) for _, d in stage_frames.values())
    okq = sum(int((d["Status"] == "OK").sum()) for _, d in stage_frames.values()
              if not d.empty)
    rwq = sum(int((d["Status"] == "Rework").sum()) for _, d in stage_frames.values()
              if not d.empty)
    rjq = sum(int((d["Status"] == "Rejection").sum()) for _, d in stage_frames.values()
              if not d.empty)
    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("Unit-stage records", total_units)
    k2.metric("OK", okq)
    k3.metric("Rework", rwq)
    k4.metric("Rejection", rjq)
    k5.metric("Reused Rejected", len(reused_win))

    if total_units == 0 and reused_win.empty:
