...
INSTALL:  py -3 -m pip install streamlit qrcode pillow openpyxl pandas plotly
...
def inject_css():
    ...
    <style>
      ...
      div[data-testid="stMetric"] {{
          background: {DECCAN_BLUE_LIGHT}; border: 1px solid #d5e2f0;
          border-radius: 8px; padding: 10px 14px;
      }}
      div[data-testid="stMetric"] label {{ color: {DECCAN_GREY}; }}
      .deccan-band {{
          background: {DECCAN_BLUE}; color: white; padding: 6px 14px;
...
def center_logo():
    """Deccan logo centered at the top of the main screen."""
    if os.path.exists(LOGO_PATH):
        l, m, r = st.columns([2, 1.2, 2])
        with m:
            st.image(LOGO_PATH, width="stretch")
    else:
        st.markdown(f"<div style='text-align:center'><span style='color:{DECCAN_BLUE};"
                    "font-weight:800;font-size:34px;letter-spacing:1px'>DECCAN</span><br>"
                    f"<span style='color:{DECCAN_BLUE};font-size:12px;letter-spacing:4px'>"
                    "— SINCE 1966 —</span></div>", unsafe_allow_html=True)
        st.caption("")  # spacing; add deccan_logo.png next to the script for the image logo
...
def page_qr_generator():
    center_logo()
    st.markdown("<span class='deccan-band'>QR GENERATOR</span>", unsafe_allow_html=True)
    st.title("Insulator QR Code Generator")
...
def prepare_data(df, do_shift_repair=True, do_md_fix=True, do_dedup=True):
    ...
    dup_df = pd.DataFrame()
    if do_dedup:
        df, dup_df = find_duplicate_rows(df, all_dt_cols)
        if not dup_df.empty:
            notes.append(f"Excluded {len(dup_df)} duplicate/test row(s) — different "
                         "Insulator ID but timestamps identical to another row. "
                         "See the 'Excluded duplicates' expander.")
...
def build_stage_frames(df, win_start, win_end):
    """For each stage return (summary_df, detail_df) restricted to the
    shift window. Rejection & Rework roll into the stage in StageAsked."""
    reason_col = df.attrs.get("reason_col")
    ...
    for s in STAGES:
        rows = []
        dtc, mcc = DT_COL[s], MC_COL.get(s)
        for _, r in df.iterrows():
            ts = r.get(dtc)
            if not in_win(ts):
                continue
            rows.append({"InsulatorID": r["Title"], "JobCard": r.get("JobCard"),
                         "MRNo": r.get("MRNo"), "SKU": r.get("SKU"),
                         "Status": "OK", "Machine": (r.get(mcc) if mcc else None) or "—",
                         "DateTime": ts, "Remarks": ""})
        stage_details[s] = rows

    # Rework -> its StageAsked stage
    for _, r in df.iterrows():
        ts = r.get("Rework_DT")
        if not in_win(ts):
            continue
        s = r.get("StageAsked")
        s = s if s in STAGES else "Moulding"
        stage_details[s].append({"InsulatorID": r["Title"], "JobCard": r.get("JobCard"),
                                 "MRNo": r.get("MRNo"), "SKU": r.get("SKU"),
                                 "Status": "Rework",
                                 "Machine": r.get("Rework_MC") or "—",
                                 "DateTime": ts, "Remarks": "Rework scan"})
    # Rejection -> its StageAsked stage
    for _, r in df.iterrows():
        ...
        s = r.get("StageAsked")
        s = s if s in STAGES else "Moulding"
        reused_flag = r["Title"] in reused_ids
        reason = (r.get(reason_col) if reason_col else None) or ""
        stage_details[s].append({"InsulatorID": r["Title"], "JobCard": r.get("JobCard"),
                                 "MRNo": r.get("MRNo"), "SKU": r.get("SKU"),
                                 "Status": "Rejection",
                                 "Machine": r.get("Rejection_MC") or "—",
                                 "DateTime": ts,
                                 "Remarks": ("REUSED LATER (QR re-run from Priming). "
                                             if reused_flag else "") + (reason or "")})

    result = {}
    for s in STAGES:
        det = pd.DataFrame(stage_details[s])
        if det.empty:
            result[s] = (pd.DataFrame(), det)
            continue
        det = det.sort_values(["JobCard", "InsulatorID", "DateTime"]).reset_index(drop=True)
        grp = det.groupby("JobCard")
        summ = pd.DataFrame({
            "Total Produced Qty": grp.size(),
            "OK Qty": grp.apply(lambda g: int((g["Status"] == "OK").sum()), include_groups=False),
            "Rework Qty": grp.apply(lambda g: int((g["Status"] == "Rework").sum()), include_groups=False),
            "Rejection Qty": grp.apply(lambda g: int((g["Status"] == "Rejection").sum()), include_groups=False),
        }).reset_index().rename(columns={"JobCard": "Job Card"})
        result[s] = (summ, det)

    # reused-rejected sheet limited to reuse OR rejection touching the window
    if not reused.empty:
        m = reused.apply(lambda r: (win_start <= r["Reuse DateTime"] < win_end) or
                                   (win_start <= r["Rejection DateTime"] < win_end), axis=1)
        reused_win = reused[m].reset_index(drop=True)
    else:
        reused_win = reused
    return result, reused_win, reused


# ======================================================================
# Excel report writer (openpyxl, formatted, COUNTIFS formulas)
# ======================================================================
def write_excel_report(stage_frames, reused_win, win_start, win_end):
    from openpyxl import Workbook
    ...
    for jc in all_jcs:
        ...
        for s in STAGES:
            ...
            r += 1
        if r > block_start:                      # subtotal row for this job card
            ov.cell(row=r, column=1, value=jc).font = sub_font
            ov.cell(row=r, column=2, value="TOTAL").font = sub_font
            for c in range(3, 7):
                col = get_column_letter(c)
                cell = ov.cell(row=r, column=c,
                               value=f"=SUM({col}{block_start}:{col}{r-1})")
                cell.font = sub_font; cell.alignment = center
            for c in range(1, 7):
                ov.cell(row=r, column=c).border = box
                ov.cell(row=r, column=c).fill = sub_fill
            r += 2                               # blank spacer between job cards
    if not all_jcs:
    ...
    for s in STAGES:
        summ, det = stage_frames[s]
        DETAIL_START = max(100, len(summ) + 12)   # never collides with summary
        ws = wb.create_sheet(STAGE_LABELS[s][:31])
        sheet_title(ws, f"{STAGE_LABELS[s]} — Production Entry", win_txt)

        ws["A4"] = "SUMMARY (Job Card level — enter these in ERP)"
        ws["A4"].font = sub_font; ws["A4"].fill = sub_fill
        heads = ["Job Card", "Total Produced Qty", "OK Qty", "Rework Qty", "Rejection Qty"]
        ...
        if not summ.empty:
            for _, row in summ.iterrows():
                jc = row["Job Card"]
                ws.cell(row=rr, column=1, value=jc).font = body_font
                # live COUNTIFS against the detail table below
                ws.cell(row=rr, column=2,
                        value=f'=COUNTIF($B${DETAIL_START}:$B$100000,A{rr})')
                ws.cell(row=rr, column=3,
                        value=f'=COUNTIFS($B${DETAIL_START}:$B$100000,A{rr},$E${DETAIL_START}:$E$100000,"OK")')
                ws.cell(row=rr, column=4,
                        value=f'=COUNTIFS($B${DETAIL_START}:$B$100000,A{rr},$E${DETAIL_START}:$E$100000,"Rework")')
                ws.cell(row=rr, column=5,
                        value=f'=COUNTIFS($B${DETAIL_START}:$B$100000,A{rr},$E${DETAIL_START}:$E$100000,"Rejection")')
                for c in range(1, 6):
        ...
        if not det.empty:
            for jc, grp_df in det.groupby("JobCard", sort=True):
                # job-card band header
                band = ws.cell(row=rr, column=1, value=f"Job Card: {jc}")
                band.font = sub_font; band.fill = sub_fill
                ...
                for _, row in grp_df.iterrows():
                    ...
                    rr += 1
                rr += 1                          # blank spacer between job cards
        for w_, col in zip([22, 14, 16, 12, 11, 11, 12, 10, 46], "ABCDEFGHI"):
...
# Charts (plotly)
# ======================================================================
def render_charts(stage_frames, reused_all, df, win_start, win_end):
    try:
        import plotly.express as px
        import plotly.graph_objects as go
    except ImportError:
        st.error("**plotly is not installed** on this Streamlit deployment. "
                 "Add a line `plotly` to the repo's `requirements.txt` "
                 "(next to streamlit, qrcode, pillow, openpyxl, pandas) and "
                 "reboot the app — charts will then work.")
        return

    palette = [DECCAN_BLUE, "#4C8BC9", "#93B8DD", "#134A85", "#7FA8CF",
               "#2E6DA4", "#B9CDE4", "#5E92C4"]
    status_colors = {"OK": "#2E8B57", "Rework": "#E8A317", "Rejection": "#C0392B"}

    all_det = pd.concat(
        [d.assign(Stage=STAGE_LABELS[s]) for s, (m, d) in stage_frames.items() if not d.empty],
        ignore_index=True) if any(not d.empty for _, d in stage_frames.values()) else pd.DataFrame()
    ...
    fdf = all_det[all_det["JobCard"].isin(sel_jc) & all_det["Stage"].isin(sel_st)
                  & all_det["Status"].isin(sel_status)]
    if fdf.empty:
        st.warning("No data after filters."); return

    cfg = {"displaylogo": False,
           "toImageButtonOptions": {"format": "png", "filename": "deccan_production_chart",
                                    "scale": 3}}
    st.caption("Use the 📷 camera icon on any chart to export it as a high-resolution PNG.")

    c1, c2 = st.columns(2)
    with c1:
        g = fdf.groupby(["Stage", "Status"]).size().reset_index(name="Qty")
        fig = px.bar(g, x="Stage", y="Qty", color="Status", barmode="stack",
                     color_discrete_map=status_colors, text="Qty",
                     title="Stage-wise Production (OK / Rework / Rejection)",
                     category_orders={"Stage": [STAGE_LABELS[s] for s in STAGES]})
        fig.update_traces(textposition="inside")
        fig.update_layout(plot_bgcolor="white", font_family="Arial",
                          title_font_color=DECCAN_BLUE_DARK, legend_title=None)
        st.plotly_chart(fig, width="stretch", config=cfg)
    with c2:
        g = fdf.groupby(["JobCard", "Status"]).size().reset_index(name="Qty")
        fig = px.bar(g, x="JobCard", y="Qty", color="Status", barmode="group",
                     color_discrete_map=status_colors, text="Qty",
    ...
    c3, c4 = st.columns(2)
    with c3:
        g = fdf[fdf["Machine"] != "—"].groupby(["Machine", "Status"]).size()\
            .reset_index(name="Qty")
        if not g.empty:
            fig = px.bar(g, x="Machine", y="Qty", color="Status", barmode="stack",
                         color_discrete_map=status_colors, text="Qty",
            ...
            st.plotly_chart(fig, width="stretch", config=cfg)
    with c4:
        ok = fdf[fdf["Status"] == "OK"].copy()
        if not ok.empty:
            ok["Hour"] = ok["DateTime"].dt.floor("h")
            g = ok.groupby(["Hour", "Stage"]).size().reset_index(name="Qty")
            fig = px.line(g, x="Hour", y="Qty", color="Stage", markers=True,
                          color_discrete_sequence=palette,
                          title="Hourly Throughput by Stage (OK scans)")
            fig.update_layout(plot_bgcolor="white", font_family="Arial",
                              title_font_color=DECCAN_BLUE_DARK, legend_title=None)
            st.plotly_chart(fig, width="stretch", config=cfg)
    ...
    c5, c6 = st.columns(2)
    with c5:
        tot = len(fdf); okq = int((fdf["Status"] == "OK").sum())
        rwq = int((fdf["Status"] == "Rework").sum())
        rjq = int((fdf["Status"] == "Rejection").sum())
        fig = go.Figure(go.Pie(labels=["OK", "Rework", "Rejection"],
                               values=[okq, rwq, rjq], hole=0.55,
                               marker_colors=[status_colors[k] for k in
                                              ["OK", "Rework", "Rejection"]],
                               textinfo="label+value+percent"))
        fig.update_layout(title="Quality Split (selected data)",
                          font_family="Arial", title_font_color=DECCAN_BLUE_DARK,
                          annotations=[dict(text=f"{tot}<br>scans", x=0.5, y=0.5,
                                            font_size=16, showarrow=False)])
        st.plotly_chart(fig, width="stretch", config=cfg)
    with c6:
        if not reused_all.empty:
            g = reused_all.groupby("Rejected At Stage").size().reset_index(name="Qty")
            fig = px.bar(g, x="Rejected At Stage", y="Qty", text="Qty",
                         color_discrete_sequence=[DECCAN_BLUE],
                         title="Reused Rejected Units by Rejection Stage (all data)")
            fig.update_layout(plot_bgcolor="white", font_family="Arial",
                              title_font_color=DECCAN_BLUE_DARK)
            st.plotly_chart(fig, width="stretch", config=cfg)
        else:
...
def page_visualizer():
    center_logo()
    st.markdown("<span class='deccan-band'>PRODUCTION ENTRY VISUALIZER</span>",
                unsafe_allow_html=True)
    st.title("Daily Production Entry Visualizer")
    st.caption("Upload the ScanLog export, pick the shift window, and generate the "
               "ERP-ready stage-wise production entry report.")
    ...
    with st.expander("Data-repair options", expanded=False):
        ...
        do_dedup = st.checkbox("Exclude duplicate/test rows (different Insulator ID, "
                               "identical timestamps to another row)", value=True)
    ...
    df, notes, dup_df = prepare_data(raw, do_shift_repair=do_shift, do_md_fix=do_md,
                                     do_dedup=do_dedup)
    for n in notes:
        st.info("🛠 " + n)
    if not dup_df.empty:
        with st.expander(f"Excluded duplicates ({len(dup_df)} rows) — review before "
                         "cleaning them out of the SharePoint list"):
            show_cols = ["Title", "DuplicateOf", "JobCard", "MRNo"]
            st.dataframe(dup_df[[c for c in show_cols if c in dup_df.columns]],
                         width="stretch", hide_index=True)
    ...
    m1.metric("Valid insulators", f"{df['Title'].nunique()}",
              delta=f"-{len(dup_df)} duplicates excluded" if not dup_df.empty else None,
              delta_color="off")
    ...
    st.caption("Shift A starts at 08:00 and Shift B ends at 08:00 next morning — "
               "select e.g. 6th → 7th to fetch everything scanned between "
               "6th 08:00 and 7th 08:00.")
    c1, c2 = st.columns([3, 1])
    rng = c1.date_input("Production date range", value=(dmin, min(dmin + timedelta(days=1), dmax)),
                        min_value=dmin - timedelta(days=1), max_value=dmax + timedelta(days=1),
                        format="DD/MM/YYYY")
    ...
    stage_frames, reused_win, reused_all = build_stage_frames(df, win_start, win_end)

    total_scans = sum(len(d) for _, d in stage_frames.values())
    okq = sum(int((d["Status"] == "OK").sum()) for _, d in stage_frames.values() if not d.empty)
    rwq = sum(int((d["Status"] == "Rework").sum()) for _, d in stage_frames.values() if not d.empty)
    rjq = sum(int((d["Status"] == "Rejection").sum()) for _, d in stage_frames.values() if not d.empty)
    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("Scans in window", total_scans)
    k2.metric("OK", okq)
    k3.metric("Rework", rwq)
    k4.metric("Rejection", rjq)
    k5.metric("Reused Rejected", len(reused_win))

    if total_scans == 0 and reused_win.empty:
    ...
    for tab, s in zip(tabs[:-1], STAGES):
        with tab:
            summ, det = stage_frames[s]
            if det.empty:
                st.caption("No records for this stage in the window.")
                continue
            ...
            show = det.copy()
            show["Date"] = show["DateTime"].dt.strftime("%d/%m/%Y")
    ...
    if st.toggle("📊 Show production charts", value=False,
                 help="Interactive dashboard for the selected window. "
                      "Every chart exports to PNG via its camera icon."):
        render_charts(stage_frames, reused_all, df, win_start, win_end)
