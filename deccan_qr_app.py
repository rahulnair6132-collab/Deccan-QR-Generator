...
def render_charts(stage_frames, reused_all):
    ...
    # ---------- Export: single self-contained HTML dashboard ----------
    st.markdown("##### Export charts")
    st.caption("Every chart above has a **⋯ menu → Save as PNG / SVG** for a single "
               "image. The button below downloads ALL charts as one dashboard file — "
               "open it in a browser and use **Print → Save as PDF** for a PDF copy.")
    try:
        dash = alt.vconcat(*charts_for_export).properties(
            title=alt.TitleParams("Deccan Enterprises — Production Analytics",
                                  fontSize=18, color="#134A85"))
        html = dash.to_html()
        st.download_button("⬇ Download charts dashboard (HTML → print to PDF)",
                           data=html.encode("utf-8"),
                           file_name="Deccan_Production_Charts.html",
                           mime="text/html", width="stretch")
    except Exception as e:
        st.warning(f"Could not build the export file: {e}")
