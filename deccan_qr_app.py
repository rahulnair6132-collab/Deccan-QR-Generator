...
def load_scan_export(uploaded_file):
        ...
        if name.endswith(".csv"):
            ...
            for enc in ("utf-8-sig", "utf-8", "cp1252", "latin-1"):
                try:
                    ...
                    last_err = e
            if df is None:
                return None, f"Could not read the CSV file: {last_err}"
        else:
    ...
    if "Title" not in df.columns:
        return None, ("The file does not look like a ScanLog export "
                      "(no 'Title' column). Columns found: "
                      + ", ".join(map(str, df.columns[:8])) + " …")
    return df, None
...
def prepare_data(df, do_shift_repair=True, do_md_fix=True, do_dedup=True):
    ...
    # explicit reason columns: RejectionReason for rejections, ReworkReason for reworks
    def _find(df_, key):
        norm = {c: "".join(ch for ch in c.lower() if ch.isalnum()) for c in df_.columns}
        return next((c for c, n in norm.items() if n == key),
                    next((c for c, n in norm.items() if key in n), None))
    df.attrs["rej_reason_col"] = _find(df, "rejectionreason") or _find(df, "reason")
    df.attrs["rw_reason_col"] = _find(df, "reworkreason")
    df.attrs["reason_col"] = df.attrs["rej_reason_col"]   # legacy consumers
...
def infer_machine_map(df):
    ...
    # majority vote: a machine belongs to the stage where it appears MOST often,
    # so a handful of wrong-machine scans can't hijack the mapping
    votes = {}
    for s, col in MC_COL.items():
        if col in df.columns:
            for m in df[col].dropna().astype(str).str.strip():
                if m and m != "—":
                    votes.setdefault(m, {}).setdefault(s, 0)
                    votes[m][s] += 1
    def by_prefix(m):
        for pref, s in DEFAULT_MACHINE_PREFIX:
            if m.upper().startswith(pref.upper()):
                return s
        return None
    # Deccan naming convention is authoritative (FP-145*=CBM, FP-165*=CAM,
    # SML*=SML, M*=Moulding, C*=C-Frame); data majority vote only for
    # machines outside the convention — so mis-scans can never flip a mapping
    mapping = {}
    for m, cnt in votes.items():
        mapping[m] = by_prefix(m) or max(cnt, key=cnt.get)
    extra = set()
...
def clean_text(v):
    """NaN/None-proof text: always returns a plain stripped str."""
...
def build_stage_frames(df, win_start, win_end, machine_map=None, shift_a_start=time(8, 0)):
    ...
    reason_col = df.attrs.get("rej_reason_col") or df.attrs.get("reason_col")
    rw_reason_col = df.attrs.get("rw_reason_col")
    ...
    for _, r in df.iterrows():
        ...
        if in_win(ts):
            ...
            if s is None:
                ...
                rec["Rework Reason"] = (clean_text(r.get(rw_reason_col))
                                        if rw_reason_col else "")
        ...
        if in_win(ts):
            ...
            if s is None:
                ...
                rec["Rejection Reason"] = (clean_text(r.get(reason_col))
                                           if reason_col else "")
    ...
    for s in STAGES:
        ...
        det["Event Machine"] = det.apply(
            lambda r: clean_text(r["Rejection Machine"]) or "—"
            if r["Status"] == "Rejection"
            else (clean_text(r["Rework Machine"]) or "—" if r["Status"] == "Rework"
                  else clean_text(r["Prod Machine"]) or "—"), axis=1)
        grp = det.groupby(["JobCard", "MRNo", "SKU"])
        summ = pd.DataFrame({
        ...
        summ["OK Qty"] = (summ["Total Produced Qty"] - summ["Rework Qty"]
                          - summ["Rejection Qty"])
        summ = summ[["Job Card", "MR No", "SKU", "Total Produced Qty", "OK Qty",
...
def write_excel_report(stage_frames, reused_win, win_start, win_end, shift_a_start=time(8, 0)):
    ...
    for s in STAGES:
        ...
        n_shift_rows = (det[det["Shift Date"] != ""]
                        .groupby(["Shift Date", "Shift", "JobCard", "MRNo", "SKU",
                                  "Event Machine"])
                        .ngroups if not det.empty else 0)
        n_mach_rows = det["Event Machine"].nunique() if not det.empty else 0
        n_rrej_rows = int(det["Status"].isin(["Rework", "Rejection"]).sum()) \
            if not det.empty else 0
        DETAIL_START = max(100, len(summ) + n_shift_rows + n_mach_rows
                           + n_rrej_rows + 38)
        ws = wb.create_sheet(STAGE_LABELS[s][:31])
        sheet_title(ws, f"{STAGE_LABELS[s]} — Production Entry", win_txt)

        ws["A4"] = "SUMMARY (Job Card + MR + SKU level — enter these in ERP)"
        ws["A4"].font = sub_font; ws["A4"].fill = sub_fill
        heads = ["Job Card", "MR No", "SKU", "Total Produced Qty", "OK Qty",
                 "Rework Qty", "Rejection Qty"]
        ...
        if not summ.empty:
            ...
            for _, row in summ.iterrows():
                ...
                # live COUNTIFS against the detail table (JobCard=B, MR=C, SKU=D, Status=E)
                base = (f'$B${D}:$B$100000,A{rr},$C${D}:$C$100000,B{rr},'
                        f'$D${D}:$D$100000,C{rr}')
                ws.cell(row=rr, column=4, value=f'=COUNTIFS({base})')
        ...
        if not det.empty:
            ...
            srow += 1
            sheads = ["Date", "Shift", "Job Card", "MR No", "SKU", "Machine",
            ...
            grouped = grouped.sort_values(["_d", "Shift", "JobCard", "MRNo", "SKU",
                                           "Event Machine"])
            for dt_txt, ddf in grouped.groupby("Shift Date", sort=False):
                ...
                for _, g in ddf.iterrows():
                    ...
                    ws.cell(row=srow, column=5, value=g["SKU"]).font = body_font
                    ws.cell(row=srow, column=6,
                            value=g["Event Machine"]).font = body_font
                    base = (f'$B${D}:$B$100000,C{srow},$C${D}:$C$100000,D{srow},'
                            f'$D${D}:$D$100000,E{srow},$P${D}:$P$100000,F{srow},'
                            f'$N${D}:$N$100000,A{d0},$O${D}:$O$100000,"{sh}"')
                    ws.cell(row=srow, column=7, value=f'=COUNTIFS({base})')
                    ws.cell(row=srow, column=8,
                            value=f'=COUNTIFS({base},$E${D}:$E$100000,"OK")')
                    ws.cell(row=srow, column=9,
                            value=f'=COUNTIFS({base},$E${D}:$E$100000,"Rework")')
                    ws.cell(row=srow, column=10,
                            value=f'=COUNTIFS({base},$E${D}:$E$100000,"Rejection")')
                    for c in range(1, 11):
            ...
            # TOTAL row across all dates/shifts
            first_data = srow - len(grouped)
            ws.cell(row=srow, column=1, value="TOTAL").font = sub_font
            for c in range(7, 11):
                col = get_column_letter(c)
                cell = ws.cell(row=srow, column=c,
                               value=f"=SUM({col}{first_data}:{col}{srow-1})")
                cell.font = sub_font; cell.alignment = center
            for c in range(1, 11):
                ws.cell(row=srow, column=c).border = box
                ws.cell(row=srow, column=c).fill = sub_fill
            srow += 1

        # ---- MACHINE-WISE SUMMARY ----
        if not det.empty:
            mrow = srow + 2
            hdr = ws.cell(row=mrow, column=1, value="MACHINE-WISE SUMMARY")
            hdr.font = sub_font; hdr.fill = sub_fill
            mrow += 1
            mheads = ["Machine", "Total Produced Qty", "OK Qty",
                      "Rework Qty", "Rejection Qty"]
            for c, h in enumerate(mheads, 1):
                ws.cell(row=mrow, column=c, value=h)
            style_header(ws, mrow, len(mheads))
            mrow += 1
            D = DETAIL_START
            for mach in sorted(det["Event Machine"].unique()):
                ws.cell(row=mrow, column=1, value=mach).font = body_font
                base = f'$P${D}:$P$100000,A{mrow}'
                ws.cell(row=mrow, column=2, value=f'=COUNTIFS({base})')
                ws.cell(row=mrow, column=3,
                        value=f'=COUNTIFS({base},$E${D}:$E$100000,"OK")')
                ws.cell(row=mrow, column=4,
                        value=f'=COUNTIFS({base},$E${D}:$E$100000,"Rework")')
                ws.cell(row=mrow, column=5,
                        value=f'=COUNTIFS({base},$E${D}:$E$100000,"Rejection")')
                for c in range(1, 6):
                    ws.cell(row=mrow, column=c).border = box
                    if c > 1:
                        ws.cell(row=mrow, column=c).alignment = center
                        ws.cell(row=mrow, column=c).font = body_font
                mrow += 1
            m_first = mrow - det["Event Machine"].nunique()
            ws.cell(row=mrow, column=1, value="TOTAL").font = sub_font
            for c in range(2, 6):
                col = get_column_letter(c)
                cell = ws.cell(row=mrow, column=c,
                               value=f"=SUM({col}{m_first}:{col}{mrow-1})")
                cell.font = sub_font; cell.alignment = center
            for c in range(1, 6):
                ws.cell(row=mrow, column=c).border = box
                ws.cell(row=mrow, column=c).fill = sub_fill
            mrow += 1
        else:
            mrow = srow + 2

        # ---- REWORK & REJECTION DETAILS (right below the summary tables) ----
        rrej = det[det["Status"].isin(["Rework", "Rejection"])] if not det.empty \
            else pd.DataFrame()
        if not rrej.empty:
            xrow = mrow + 2
            hdr = ws.cell(row=xrow, column=1,
                          value="REWORK & REJECTION DETAILS — machines and reasons")
            hdr.font = sub_font; hdr.fill = sub_fill
            xrow += 1
            rheads = ["Insulator ID", "Type", "Machine", "Stage", "Job Card",
                      "MR No", "Date/Time", "Reason / Remarks"]
            for c, h in enumerate(rheads, 1):
                ws.cell(row=xrow, column=c, value=h)
            style_header(ws, xrow, len(rheads))
            xrow += 1
            for _, row in rrej.sort_values(["Status", "InsulatorID"]).iterrows():
                is_rej = row["Status"] == "Rejection"
                machine = clean_text(row["Rejection Machine"] if is_rej
                                     else row["Rework Machine"]) or "—"
                ts_ = (row["Rejection DateTime"] if is_rej
                       else row["Rework DateTime"])
                reason = " — ".join(
                    x for x in [clean_text(row["Rejection Reason"]) if is_rej
                                else clean_text(row["Rework Reason"]),
                                clean_text(row["Remarks"])] if x) or "—"
                vals = [row["InsulatorID"], row["Status"], machine,
                        STAGE_LABELS[s], row["JobCard"], row["MRNo"],
                        (ts_.strftime("%d/%m/%Y %H:%M") if pd.notna(ts_) else ""),
                        reason]
                for c, v in enumerate(vals, 1):
                    cell = ws.cell(row=xrow, column=c, value=v)
                    cell.font = body_font; cell.border = box
                    if c in (2, 3, 4, 7):
                        cell.alignment = center
                ws.cell(row=xrow, column=2).font = Font(
        ...
        if not det.empty:
            for jc, grp_df in det.groupby("JobCard", sort=True):
                ...
                for _, row in grp_df.iterrows():
                    reason = clean_text(
                        row["Rejection Reason"] if row["Status"] == "Rejection"
                        else (row["Rework Reason"] if row["Status"] == "Rework" else ""))
                    remarks = clean_text(row["Remarks"])
        ...
        for w_, col in zip([22, 13, 15, 11, 11, 12, 11, 9, 13, 16, 15, 17, 34, 12, 7, 12],
...
# ======================================================================
def render_charts(stage_frames, reused_win):
    ...
    rank = {"OK": 0, "Rework": 1, "Rejection": 2}
    AX = dict(labelFontSize=12, titleFontSize=13, titleColor="#134A85")
    GRID = dict(grid=True, gridColor="#dbe6f2", **AX)
    CHART_W, CHART_H = 660, 360
    ...
    def stacked_bar(data, xcol, x_title, title, x_order=None):
        """Stacked bar with NON-overlapping labels: segment values centred
        INSIDE their segment, total printed once above the bar."""
        g = (data.groupby([xcol, "Status"]).size().reset_index(name="Qty"))
        # cumulative mid-position per segment for in-bar labels
        g["_r"] = g["Status"].map(rank)
        g = g.sort_values([xcol, "_r"])
        g["_cum"] = g.groupby(xcol)["Qty"].cumsum()
        g["_mid"] = g["_cum"] - g["Qty"] / 2
        tot = g.groupby(xcol)["Qty"].sum().reset_index(name="Total")
        x_enc = alt.X(f"{xcol}:N", sort=x_order, title=x_title,
                      axis=alt.Axis(labelAngle=-25, labelLimit=180, **AX))
        bars = alt.Chart(g).mark_bar(size=52).encode(
            x=x_enc,
            y=alt.Y("Qty:Q", title="Units", axis=alt.Axis(**GRID)),
            color=alt.Color("Status:N", scale=status_scale,
                            legend=alt.Legend(title="Status")),
            order=alt.Order("_r:Q"),
            tooltip=[xcol, "Status", "Qty"])
        min_seg = max(2, int(g.groupby(xcol)["Qty"].sum().max() * 0.04))
        seg_lbl = alt.Chart(g[g["Qty"] >= min_seg]).mark_text(
            color="white", fontWeight="bold", fontSize=13).encode(
            x=x_enc, y=alt.Y("_mid:Q"), text="Qty:Q",
            order=alt.Order("_r:Q"))
        tot_lbl = alt.Chart(tot).mark_text(
            dy=-8, fontWeight="bold", fontSize=13, color="#134A85").encode(
            x=x_enc, y=alt.Y("Total:Q"), text="Total:Q")
        return (bars + seg_lbl + tot_lbl).properties(
            title=title, width=CHART_W, height=CHART_H)

    # ---------- 1. Per-stage quality donuts ----------
    st.markdown("##### Stage-wise Quality Split")
    st.caption("One donut per stage — unit counts on the slices, stage total "
               "in the centre.")
    ...
    for s_lbl in [x for x in stage_order if x in set(fdf["Stage"])]:
        ...
        base = alt.Chart(g).encode(
            ...
            tooltip=["Status", "Qty"])
        arc = base.mark_arc(innerRadius=46, outerRadius=74)
        labels = base.mark_text(radius=92, fontWeight="bold",
    ...
    for i in range(0, len(donuts), 3):
        row = alt.hconcat(*donuts[i:i + 3]).resolve_scale(color="shared")
        register(row)
        st.altair_chart(row, width="stretch")

    # ---------- 2. Stage-wise stacked bar ----------
    ch = stacked_bar(fdf, "Stage", "Production Stage",
                     "Units at each Stage (OK / Rework / Rejection)",
                     x_order=stage_order)
    register(ch); st.altair_chart(ch, width="stretch")

    # ---------- 3. Job Card — unit-level ----------
    ...
    x_enc = alt.X("JobCard:N", title="Job Card",
                  axis=alt.Axis(labelAngle=0, labelLimit=180, **AX))
    bars = alt.Chart(g).mark_bar(size=44).encode(
        x=x_enc, y=alt.Y("Qty:Q", title="Distinct Insulators",
                         axis=alt.Axis(**GRID)),
        xOffset=alt.XOffset("Status:N", sort=STATUS_ORDER),
        color=alt.Color("Status:N", scale=status_scale,
                        legend=alt.Legend(title="Final Status")),
        tooltip=["JobCard", "Status", "Qty"])
    lbl = alt.Chart(g).mark_text(dy=-8, fontWeight="bold", fontSize=13).encode(
        x=x_enc, xOffset=alt.XOffset("Status:N", sort=STATUS_ORDER),
        y=alt.Y("Qty:Q"), text="Qty:Q")
    ch = (bars + lbl).properties(
        title="Job Card — Distinct Units by Final Status (counted once)",
        width=CHART_W, height=CHART_H)
    register(ch); st.altair_chart(ch, width="stretch")

    # ---------- 4. Machine-wise ----------
    mdf = fdf[fdf["Event Machine"].astype(str).str.strip().ne("") &
              fdf["Event Machine"].ne("—")].rename(
        columns={"Event Machine": "Machine"})
    if not mdf.empty:
        ch = stacked_bar(mdf, "Machine", "Machine",
                         "Machine-wise Output (all stages combined)")
        register(ch); st.altair_chart(ch, width="stretch")
        # cross-stage composition note when a machine serves multiple stages
        comp = mdf.groupby("Machine")["Stage"].nunique()
        multi = comp[comp > 1].index.tolist()
        if multi:
            det_txt = "; ".join(
                f"**{m}**: " + ", ".join(
                    f"{st_} ({n})" for st_, n in
                    mdf[mdf['Machine'] == m].groupby('Stage').size().items())
                for m in multi)
            st.caption("ℹ This chart sums a machine across ALL stages — "
                       f"machines appearing at multiple stages: {det_txt}. "
                       "Per-stage machine tables are in the Excel report.")

    # ---------- 5. Hourly throughput (value labels on points) ----------
    ...
    if not okd.empty:
        ...
        base = alt.Chart(g).encode(
            ...
            y=alt.Y("Qty:Q", title="Units scanned",
                    scale=alt.Scale(padding=14), axis=alt.Axis(**GRID)),
            color=alt.Color("Stage:N", legend=alt.Legend(title="Stage"),
                            sort=stage_order))
        line = base.mark_line(point=alt.OverlayMarkDef(size=64)).encode(
            tooltip=[alt.Tooltip("Hour:T", format="%d/%m/%Y %H:%M"),
                     "Stage", "Qty"])
        pts = base.mark_text(dy=-11, fontSize=11, fontWeight="bold").encode(
            text="Qty:Q")
        ch = (line + pts).properties(title="Hourly Throughput by Stage",
                                     width=CHART_W, height=CHART_H)
        register(ch); st.altair_chart(ch, width="stretch")

    # ---------- 6. ALL rejections & reworks by stage ----------
    rrdf = fdf[fdf["Status"].isin(["Rework", "Rejection"])]
    if not rrdf.empty:
        g = (rrdf.groupby(["Stage", "Status"]).size().reset_index(name="Qty"))
        x_enc = alt.X("Stage:N", sort=stage_order, title="Stage",
                      axis=alt.Axis(labelAngle=-25, labelLimit=180, **AX))
        bars = alt.Chart(g).mark_bar(size=34).encode(
            x=x_enc, y=alt.Y("Qty:Q", title="Units", axis=alt.Axis(**GRID)),
            xOffset=alt.XOffset("Status:N", sort=["Rework", "Rejection"]),
            color=alt.Color("Status:N",
                            scale=alt.Scale(domain=["Rework", "Rejection"],
                                            range=["#E8A317", "#C0392B"]),
                            legend=alt.Legend(title=None)),
            tooltip=["Stage", "Status", "Qty"])
        lbl = alt.Chart(g).mark_text(dy=-8, fontWeight="bold",
                                     fontSize=13).encode(
            x=x_enc, xOffset=alt.XOffset("Status:N", sort=["Rework", "Rejection"]),
            y=alt.Y("Qty:Q"), text="Qty:Q")
        ch = (bars + lbl).properties(
            title="ALL Reworks & Rejections by Stage (matches the donuts above)",
            width=CHART_W, height=300)
        register(ch); st.altair_chart(ch, width="stretch")

    # ---------- 7. Reused rejected (subset of rejections) ----------
    if not reused_win.empty:
        g = reused_win.groupby("Rejected At Stage").size().reset_index(name="Qty")
        base = alt.Chart(g).encode(
            x=alt.X("Rejected At Stage:N", title="Stage where rejection happened",
                    axis=alt.Axis(labelAngle=-25, labelLimit=180, **AX)),
            y=alt.Y("Qty:Q", title="Units", axis=alt.Axis(**GRID)),
            tooltip=["Rejected At Stage", "Qty"])
        ch = (base.mark_bar(size=52, color=DECCAN_BLUE)
              + base.mark_text(dy=-8, fontWeight="bold").encode(text="Qty:Q")
              ).properties(title="Reused-Rejected Units by Rejection Stage "
                                 "(subset of rejections: rubber peeled, QR re-run)",
                           width=CHART_W, height=300)
        register(ch); st.altair_chart(ch, width="stretch")
        st.caption("Note: this chart is a SUBSET of the rejections above — only "
                   "units that were rejected and later re-run from Priming on the "
                   "same QR. A stage can have rejections here without them being "
                   "reused (and vice versa).")

    # ---------- Export: ALL charts, A4-paged PDF or single PNG ----------
    st.markdown("##### Export all charts")
    st.caption("One click composes every chart above — PNG gives one tall image, "
               "PDF gives a clean A4 document with one chart per page.")
    ...
    <div style="font-family:'Source Sans Pro',Arial,sans-serif">
      ...
      <button id="btnPdf" style="background:#134A85;color:#fff;border:none;
              border-radius:6px;padding:10px 18px;font-weight:600;cursor:pointer">
              ⬇ Download ALL charts (PDF, A4)</button>
    ...
    async function renderImgs() {{
      ...
      for (let i = 0; i < SPECS.length; i++) {{
        ...
        const url = await res.view.toImageURL('png', 2.5);
        imgs.push(await new Promise((ok, err) => {{
          const im = new Image(); im.onload = () => ok(im);
    ...
    document.getElementById('btnPng').onclick = async () => {{
      try {{
        const imgs = await renderImgs();
        const PAD = 50, HEAD = 120;
        ...
        ctx.fillStyle = '#134A85'; ctx.font = 'bold 42px Arial';
        ctx.textAlign = 'center'; ctx.fillText(TITLE, w / 2, 60);
        ctx.fillStyle = '#6B7280'; ctx.font = '24px Arial';
    ...
    document.getElementById('btnPdf').onclick = async () => {{
      try {{
        const imgs = await renderImgs();
        // A4 portrait, flow layout: stack 2-3 charts per page
        const PW = 595.28, PH = 841.89, M = 36, GAP = 18;
        const pdf = new jspdf.jsPDF({{orientation: 'p', unit: 'pt',
                                     format: 'a4', compress: true}});
        pdf.setTextColor(19, 74, 133);
        pdf.setFont('helvetica', 'bold'); pdf.setFontSize(16);
        pdf.text(TITLE, PW / 2, M + 6, {{align: 'center'}});
        pdf.setTextColor(107, 114, 128);
        pdf.setFont('helvetica', 'normal'); pdf.setFontSize(9);
        pdf.text(SUB, PW / 2, M + 22, {{align: 'center'}});
        let y = M + 40;
        const availW = PW - M * 2;
        const maxH = (PH - M * 2 - 2 * GAP) / 3.05;   // 3 charts per page with margin
        for (const im of imgs) {{
          let scale = Math.min(availW / im.width, maxH / im.height);
...
def page_visualizer():
    ...
    # scans recorded on a machine that belongs to a DIFFERENT process
    anomalies = []
    for s in STAGES:
        _, det_s = stage_frames[s]
        if det_s.empty:
            continue
        for _, r_ in det_s[det_s["Status"] == "OK"].iterrows():
            m_ = str(r_["Prod Machine"]).strip()
            if m_ and m_ != "—" and m_ in mmap and mmap[m_] != s:
                anomalies.append({"InsulatorID": r_["InsulatorID"],
                                  "JobCard": r_["JobCard"],
                                  "Scanned Stage": STAGE_LABELS[s],
                                  "Machine": m_,
                                  "Machine belongs to": STAGE_LABELS[mmap[m_]],
                                  "Scan DateTime": r_["Prod DateTime"]})
    if anomalies:
        adf = pd.DataFrame(anomalies)
        st.warning(f"⚠ Machine/stage mismatch: {len(adf)} scan(s) recorded on a "
                   "machine mapped to a different process (likely wrong machine "
                   "selected at scan time). These are counted at the scanned "
                   "stage but inflate that machine's cross-stage total.")
        with st.expander("View machine/stage mismatches"):
    ...
    for tab, s in zip(tabs[:-1], STAGES):
        with tab:
            ...
            st.markdown("**Machine-wise summary**")
            mg = (det.groupby("Event Machine")["Status"]
                  .agg(Total="size",
                       OK=lambda g: int((g == "OK").sum()),
                       Rework=lambda g: int((g == "Rework").sum()),
                       Rejection=lambda g: int((g == "Rejection").sum()))
                  .reset_index().rename(columns={"Event Machine": "Machine"}))
            ...
            st.dataframe(show[["InsulatorID", "JobCard", "MRNo", "SKU", "Status",
                               ...
                               "Rework DateTime", "Rework Reason",
                               "Rejection Machine", "Rejection DateTime",
                               "Rejection Reason", "Remarks"]],
                         width="stretch", hide_index=True)
            ...
            if not rrej.empty:
                ...
                rt = pd.DataFrame({
                    ...
                    "Reason / Remarks": rrej.apply(
                        lambda r: " — ".join(
                            x for x in [clean_text(r["Rejection Reason"])
                                        if r["Status"] == "Rejection"
                                        else clean_text(r["Rework Reason"]),
                                        clean_text(r["Remarks"])] if x) or "—",
                        axis=1),
                })
    ...
    if st.toggle("📊 Show production charts", value=False,
        ...
        render_charts(stage_frames, reused_win)
